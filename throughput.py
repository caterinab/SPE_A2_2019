...
while (i <= 1510):
    ...
    with open(file) as data:
        ...
        with open('throughput.csv', mode='a', newline='') as out:
            ...
            print(str(j+1) + "\t" + 
                str(round(throughput[j] / 30 * 8 / 1024/1024, 3)) + "\t\t" +
                str(round(collided[j]/(collided[j] + received[j]), 4)) + "\t\t" +
                str(round(dropped[j]/generated[j],4)) + "\t\t" +
                str(round(total_offered_load / 30 * 8 / 1024/1024 / 8, 3)))
