...
class Log:
    ...
    LOG_RECEIVED = Packet.PKT_RECEIVED #1
    # packet has been corrupted due to, for example, a collision
    LOG_CORRUPTED = Packet.PKT_CORRUPTED #2
    # packet has just been generated
    LOG_GENERATED = LOG_CORRUPTED + 1 #3
    # packet has been dropped because there is no space in the queue
    LOG_QUEUE_DROPPED = LOG_GENERATED + 1 #4
    # use to log queue size in time
    LOG_QUEUE_SIZE = LOG_QUEUE_DROPPED + 1 #5
    # use to log node state in time
    LOG_NODE_STATE = LOG_QUEUE_SIZE + 1 #6
    # use to log carrier sensing state
    LOG_SENSING = LOG_NODE_STATE + 1 #7
    LOG_COLLIDED = LOG_NODE_STATE + 1 #8
